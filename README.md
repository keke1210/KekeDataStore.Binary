...
KekeDataStore.Binary is a simple data storage library that stores data into binary files. Also, it can be used as an in-memory database (NuGet Package) .NET Standard 2.0.
...
```
And a file with the file name ``Contacts.bin`` will be generated automatically by default, on client's ``/bin/Debug`` project path.
